                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <div class="nav-tabs-custom no-shadow">
                            ...
                            <br>
                            <form action="{{ route('admin.blog.update', $article->id) }}" method="post" enctype="multipart/form-data" data-parsley-validate="" class="form-horizontal form-label-left">
                                @csrf
                                @method('PATCH')
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                <select name="groups[]" id="groups" class="form-control" dir="rtl" multiple>
                                                    ...
                                                    @foreach($groups as $id => $name)
                                                        <option value="{{ $id }}"{{ (in_array($id, old('groups') ?? $selectedGroups) ? 'selected' : '') }}>{{ $name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div id="tab_information" class="tab-pane">
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                ...
                                                @if($article->image_id)
                                                       data-default-file="{{ $article->image->name }}"
                                                @endif
                                                >
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                <div id="gender" class="btn-group" data-toggle="buttons">
                                                    <label class="btn btn-default" data-toggle-class="btn-primary" data-toggle-passive-class="btn-default">
                                                        <input type="radio" name="status" value="0" data-parsley-multiple="status"{{ (old('status', $article->status) == 0 ? ' checked' : '') }}> پیش‌نویس
                                                    </label>
                                                    <label class="btn btn-primary" data-toggle-class="btn-primary" data-toggle-passive-class="btn-default">
                                                        <input type="radio" name="status" value="1" data-parsley-multiple="status"{{ (old('status', $article->status) == 1 ? ' checked' : '') }}> انتشار
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
