...
function dragNdrop(event) {
    ...
    var uploadFileInput = $('#uploadFile').clone().attr('id', '').attr('name', 'images[]').appendTo('.dragBox');
}
function drag() {
...
function drop() {
 ...
 $(document).ready(function() {
  ...
  function filterCities() {
    ...
    input = document.getElementById('searchCities');
    filter = input.value.toUpperCase();
    ul = document.getElementById("selectedCities");
...
function filterHomeCities() {
    var input, filter, ul, li, a, i, txtValue;
    input = document.getElementById('searchHomeCities');
    filter = input.value.toUpperCase();
    ul = document.getElementById("selectedHomeCities");
    li = ul.getElementsByTagName('li');
    ulp = document.getElementById("selectedPopularCities");
    lip = ulp.getElementsByTagName('li');
    ...
    for (i = 0; i < lip.length; i++) {
        a = lip[i].getElementsByTagName("a")[0];
        txtValue = a.textContent || a.innerText;
        if (txtValue.toUpperCase().indexOf(filter) > -1) {
            lip[i].style.display = "";
        } else {
            lip[i].style.display = "none";
        }
    }
}